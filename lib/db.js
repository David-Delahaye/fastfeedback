...
export function createFeedback(data,site) {
  const feedback = firestore.collection('feedback').doc();
  feedback.set(data);

  //add count to siteOwner
  const userRef = firestore.collection('users').doc(site.authorId);
  userRef.update({feedbackCount: increment});  

  return feedback;
}

export function deleteFeedback(id, user) {
  const removed = firestore.collection('feedback').doc(id).update({ status: 'removed' });

    //remove count to siteOwner
    const userRef = firestore.collection('users').doc(user.uid);
    userRef.update({feedbackCount: decrement}); 
